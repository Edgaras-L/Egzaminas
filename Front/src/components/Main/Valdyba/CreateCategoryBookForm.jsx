...
import { addNewCategoryBook } from '../../../api/lib/CategoryBookAPI';

function CreateCategoryBookForm({ handlepopupClose, render, setRender, userId }) {
    ...
    const onSubmit = async (data) => {
        ...
        await addNewCategoryBook(data).then(() => { setRender(!render) })   //send data into database(depending on current UserId)
        ...
        <div className='popupform d-flex flex-column flex-nowrap'>
            <div className='formblock'>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='d-flex flex-column flex-wrap text-center'>
                    ...
                    <p className=' p-0 text-danger'>{errors.categoryBook?.message}</p>
                    <div className='formfooter d-flex flex-row flex-wrap mt-5'>
                        <div className='me-4'>
                            <button
                                className='w-55 btn text-light'
                                type='submit' id="btn" disabled={!categoryBook}>Sukurti
                            </button>
                        </div>
                        <div className='me-4'>
                            <button
...
export default CreateCategoryBookForm